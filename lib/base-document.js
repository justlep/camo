...
export class BaseDocument {
    ...
    /**
     * set schema
     * TODO deprecate this method. make it static instead
     * @param {Object} extension
     */
    schema(extension) {
        if (schemaByDocClass.has(this.constructor)) {
            return;
            //throw new Error(`Illegal invocation - a schema for class ${this.constructor.name} has already been created`);
        }
    ...
    validate() {
        for (const key of this._schema[SCHEMA_ALL_KEYS]) {
            ...
            if (isArrayTypeEntry) {
                ...
                if (value.constructor !== schemaType && !(schemaType === Date && isNumber(value))) {
                ...
                if (!(value instanceof schemaType) && !isNativeId(value)) {
                    throw new ValidationError(this, key, `should be ${schemaType.name}, but is ${typeof value}`);
                }
                // TODO maybe ensure that the given Document instance already has an _id?
            } else {
     ...
     * TODO think about cached schema array (behind symbol) of date-key candidates, i.e. Date|Date[]|Embedded/Embedded[] w/ Dates
     *   Symbol should be named SCHEMA_DATE_KEYS
     */
    canonicalize() {
        const schema = this._schema;
        for (const schemaEntry of schema[SCHEMA_ALL_ENTRIES_NO_ID]) {
            let key = schemaEntry._key,
                value = this[key];

            if (schemaEntry.type === Date && isNumber(value)) {
                this[key] = new Date(value);
            } else if (!value) {
                continue;
            } 
            if (schemaEntry[ST_IS_TYPED_ARRAY]) {
                if (!isArray(value) || !value.length) {
                    continue;
                }
                if (schemaEntry.elementType === Date) {
    ...
    static _fromData(data) {
        ...
        for (const obj of isArray(data) ? data : [data]) {
            ...
            for (const key of Object.keys(obj)) {
                ...
                if (!schemaEntry) {
                    // TODO no log.warn that there's some data we're probably gonna lose by the next save() + find()?
                    instance[key] = objVal; // If its not in the schema, we don't care about it... right?
                    continue; 
                }
                
                // TODO use default not also if objVal === undefined?
     ...
     * EmbeddedDocument values are converted to POJOs, while
