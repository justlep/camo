...
const REF_EMBD_KEYS = Symbol('refEkeys');
...
export class BaseDocument {
    ...
    constructor() {
        ...
        this._schema._id = {type: DB().nativeIdType()};
        this._id = null;
    }
     ...
     * 
     * TODO this methods should not be called for every single new instance, but result cached on the constructor! 
     */
    generateSchema() {
        ...
        /** @type {Set<string>} - contains keys which reference single or arrays of Embedded-type entities */
        this._schema[REF_EMBD_KEYS] = new Set();
        ...
        for (const key of Object.keys(this)) {
            ...
            if (propVal.type) {
                this._schema[key] = propVal;
            } else if (isSupportedType(propVal)) {
                this._schema[key] = {type: propVal};
            } else {
            ...
            if (isArray(schemaType)) {
                ...
                if (schemaType[0]?.[IS_DOCUMENT]) {
                    ...
                    this._schema[REF_EMBD_KEYS].add(key);
                }
            } else {
                this[key] = this.getDefault(key);
    ...
    static _instantiate() {
        ...
        instance.generateSchema();  // TODO generating the same fix schema over and over again for each instance??
        return instance;
    }
    ...
    _getEmbeddeds() {
        let embeddeds = [];
        for (const key of this._schema[REF_EMBD_KEYS]) {
