...
export class NeDbClient extends DatabaseClient {
     ...
     *
     * @param {String} url
     * @param {typeof Datastore} datastoreClass
     * @returns {Promise}
     */
    static connect(url, datastoreClass) {
        ...
        if (Datastore && datastoreClass !== Datastore) {
            throw new Error('Cannot change Datastore class after first connect');
        }
 ...
 *
 * @param {String} url - example: "nedb://path/to/file/folder"
 ...
 */
export function connect(url, datastoreClass) {
    if (url.startsWith('nedb://')) {
        return NeDbClient.connect(url, datastoreClass);
    }
    throw Promise.reject('Unrecognized DB connection url.');
}
