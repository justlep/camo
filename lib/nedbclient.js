...
class NeDbClient extends DatabaseClient {
    ...
    /**
     * @param {string} url - 'nedb://memory' for in-memory, otherwise 'nedb:///path/to/dbFileFolder' 
     */
    constructor(url) {
        super(url);
    ...
    findOneAndUpdate(collection, query, values, options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.findOne(collection, query).then(data => {
                ...
                db.update(query, {[options.unset ? '$unset' : '$set']: values}, (error, result) => {
                    if (error) {
                        return reject(error);
                    }
     ...
     * (!) Behavior for persistent NeDBs is inconsistent since NeDB's async executor might still
     *     be about to write some $$indexCreated info to a .db file while we are deleting that very file here.
     *     That's why _dropDatabase() is now marked @internal, to be used for testing only; 
     *     Also, in tests, make sure to await-save some data if creating Document instances of schemas
     *           that include unique indexes; otherwise tests may fail erratically.
     *
     * @returns {Promise}
     * @internal
     */
