...
class NeDbClient extends DatabaseClient {
     ...
     *
     * @param {String} url
     * @param {CamoNedbConnectOptions} opts
     * @returns {Promise}
     */
    static connect(url, opts = {}) {
        ...
        let customDatastore = opts.NedbDatastoreClass;
        if (customDatastore) {
            if (Datastore && customDatastore !== Datastore) {
                throw new Error('Datastore already set. Cannot be changed after first connect');
            }
            if (typeof customDatastore !== 'function' || typeof customDatastore.prototype.loadDatabase !== 'function') {
                throw new Error('Given NedbDatastoreClass option is no NeDB Datastore class');
            }
            Datastore = customDatastore;
        } else if (!Datastore) {
            Datastore = require('nedb');  // default to optional dependency's NeDB Datastore class
        }
