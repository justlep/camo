...
export class Document extends BaseDocument {
     ...
     * @param {boolean} [shouldCompactIfPurged] if true, the data file will be compacted if any docs where changed
     * @return {Promise<{totalUpdates: number, ids: ?string[]}>}
     */
    static async purgeObsoleteProperties(propNames, returnIds, shouldCompactIfPurged) {
        const propsUnsetMap = Object.create(null);
        ...
        try {
            ...
            while (doc = await this.findOneAndUpdate(query, propsUnsetMap, {unset: true, populate: false})) {
                for (const name of propNames) {
                    delete doc[name];
                }
                totalUpdates++;
            ...
            if (totalUpdates && shouldCompactIfPurged) {
                await new Promise((resolve, reject) => 
                    DB()._getDb(this.collectionName()).persistence.compactDatafile(err => err ? reject(err) : resolve()));
            }
        } finally {
